window.CuaHangConTroller = function ($http, $scope, $filter) {
  $scope.listCH = [];
  $scope.listMS = [];
  $scope.listKC = [];
  $scope.listCL = [];
  ...
  $scope.loadDataCuaHang = function () {
    $scope.searchThuocTinh = function () {
      ...
      var apiUrl = cuaHangApi + "/hien-thi?page=" + $scope.page + "&";
      ...
      $http.get(apiUrl).then(function (response) {
        ...
        // $scope.listPage = Math.ceil($scope.totalPages / 12) - 1;
        console.log("aaaaaaaaaaaaa",$scope.totalPages, response.data);
      });
    };
  ...
  $scope.loadMauSac = function () {
    $http.get(cuaHangApi + "/mau-sac").then(function (response) {
  ...
  $scope.loadKichCo = function () {
    $http.get(cuaHangApi + "/kich-co").then(function (response) {
  ...
  $scope.loadTheLoai = function () {
    $http.get(cuaHangApi + "/the-loai").then(function (response) {
  ...
  $scope.loadChatLieu = function () {
    $http.get(cuaHangApi + "/chat-lieu").then(function (response) {
  ...
  $scope.nextPages = function () {
    if ($scope.page > Math.ceil($scope.totalPages / 12)-1) {
      $scope.page = Math.ceil($scope.totalPages / 12)-1 ;
    }
    $scope.page++;
    $scope.loadDataCuaHang();
  };
