window.chatLieuController = function ($scope, $http, $routeParams, $location) {
  $scope.listCL = [];
  // $scope.listCV = [];
  ...
  //load Data
  $scope.loadData = function () {
    $http.get(chatLieuApi + "/getAll?page=" + $scope.pageNo).then(
      function (response) {
        ...
        $scope.totalPages = response.data.totalPages;
        $scope.listCL = response.data.content;
      },
      function (err) {
        console.log(err);
      }
  ...
  $scope.add = function (event) {
    ...
    $http.post(chatLieuApi + "/add", $scope.x).then(
      function (response) {
  ...
  function isDuplicateName(name) {
    for (var i = 0; i < $scope.listCL.length; i++) {
      if ($scope.listCL[i].ten === name) {
        return true;
      }
    }
    return false;
  ...
  $scope.delete = function (event, id) {
    event.preventDefault();
    $http.delete(chatLieuApi + "/delete/" + id).then(
      function (response) {
  ...
  $scope.update = function (event) {
    ...
    let idCL = $scope.listCL[$scope.vitriUpdate];
    ...
    $http.put(chatLieuApi + "/update/" + idCL.id, $scope.x).then(
      function (response) {
