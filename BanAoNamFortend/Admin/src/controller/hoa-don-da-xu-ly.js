window.hoaDonXuLy = function ($scope, $http, $routeParams, $location) {
  $scope.listHD = [];
  $scope.listHDHT = [];
  $scope.listHDCT = [];
  ...
  $scope.loadSearchTT = function () {
    ...
    $http
      .get(
        hoaDonApi +
          "/search-trang-thai-huy-hoan?trangThaiS=6&pageNo=" +
          $scope.pageNo
      )
      .then(
        function (response) {
  ...
  $scope.timKiem = function () {
    $http
      .get(hoaDonApi + "/tim-kiem?trangThai=5&searchByMa=" + $scope.search)
